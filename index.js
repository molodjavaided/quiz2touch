document.addEventListener('DOMContentLoaded', function() {
    ...
    const buttonsFooter = document.querySelector('.buttons-from__footer');
    ...
    showStyleBtn.addEventListener('click', async function() {
        if (selectedStyle) {
            selectedStyleName.textContent = selectedStyle.name;
            currentStep = 10;
            showStep(currentStep);
            const examples = await loadStyleExamples(selectedStyle.id);
    ...
    form.addEventListener('input', (e) => {
        if (e.target.tagName === 'INPUT' && currentStep > 0 && currentStep < 9) {
